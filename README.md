...
BackgroundImage.Image = "rbxassetid://98380518861550"
...
BarBG.Position = UDim2.new(0.2,0,0.85,0)  -- Muito mais baixo, quase no fim da tela
BarBG.BackgroundColor3 = Color3.fromRGB(80, 0, 0)
BarBG.BorderSizePixel = 2
BarBG.BorderColor3 = Color3.fromRGB(255, 255, 255)  -- Contorno branco
BarBG.Parent = Frame
...
-- Texto de porcentagem abaixo da barra
local Percent = Instance.new("TextLabel")
Percent.Size = UDim2.new(0.6,0,0.05,0)
Percent.Position = UDim2.new(0.2,0,0.92,0)  -- Abaixo da barra (0.85 + 0.07)
...
-- Título "Slenderman Hub" - AGORA LOGO ACIMA DA BARRA
local Label = Instance.new("TextLabel")
Label.Size = UDim2.new(0.6,0,0.08,0)
Label.Position = UDim2.new(0.5,0,0.75,0)  -- Logo acima da barra (0.85 - 0.10)
Label.AnchorPoint = Vector2.new(0.5,0)
Label.BackgroundTransparency = 1
Label.Text = "Slenderman Hub"
...
task.spawn(function()
	...
	while ScreenGui.Parent do
		...
		-- Movimento vertical menor para não se afastar muito da barra
		Label.Position = UDim2.new(0.5,0,0.75+0.005*math.sin(t*1.2),0)
		task.wait(0.03)
	end
end)
...
local Window = redzlib:MakeWindow({
    Title = Translator:traduzir("Slenderman Hub"),
    SubTitle = "By:Slenderman",
    SaveFolder = "redz Hub | Brookhaven RP.lua"
})

Window:AddMinimizeButton({
	Button = {
		Image = "rbxassetid://122697101352012"
	},
	Corner = {
...
local args = {
    [1] = "RolePlayName",
    [2] = "Usuário do Hub: [Slenderman Hub]"
}

game:GetService("ReplicatedStorage").RE:FindFirstChild("1RPNam1eTex1t"):FireServer(unpack(args))
local function copyDiscordLink()
    setclipboard ("https://discord.gg/GRmMFnM2")
end
...
local predefinedMessages = {
    ...
    {"E o Bonde da Phantom", "E o Bonde da Phantom"},
...
local nomesRP = {
    ...
    "XALALALAL", "ANALFABETO", "Criança", "Web namoro", "Veia", "Phantom Client", 
...
Tabs.TT:AddButton({
    Name = "Haki Do Rei Créditos: Drip Client",
    Callback = function()
        _G.HakiAtived = true
        _G.StopHaki = false
...
Tabs.AD:AddSection({Translator:traduzir("Músicas Especiais - SERVIDOR")})
...
-- Ordenar as músicas por nome para ficar mais organizado
...
-- Função para tocar áudio no servidor (todos ouvem)
    ...
    if RE then
        ...
        if soundEvent then
            pcall(function()
                ...
                print("🎵 Áudio enviado para o servidor: " .. audioId)
            end)
        else
            warn("❌ RemoteEvent 1Gu1nSound1s não encontrado!")
        end
    else
        warn("❌ Pasta RE não encontrada no ReplicatedStorage!")
    end
end

-- Função para tocar áudio localmente
local function playAudioLocal(audioId)
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        print("🔊 Áudio tocando localmente: " .. audioId)
    else
        warn("❌ Character ou HumanoidRootPart não encontrado!")
    end
end

Tabs.AD:AddDropdown({
    Name = Translator:traduzir("Selecionar Música Especial"),
    Description = Translator:traduzir("🎵 " .. #specialMusicNames .. " músicas disponíveis - TODOS OUVEM"),
...
Tabs.AD:AddButton({
    Name = Translator:traduzir("▶️ Tocar Música (SERVIDOR)"),
    ...
    Callback = function()
        if selectedSpecialMusic then
            ...
            -- Tocar localmente também
            playAudioLocal(selectedSpecialMusic)
            
            redzlib:Notify({
                Title = "🎵 Tocando Música",
                Description = "Música tocando para todo o servidor!",
                Type = "Success",
                Duration = 3
            })
        else
            redzlib:Notify({
                Title = "❌ Erro",
                Description = "Selecione uma música primeiro!",
                Type = "Error",
                Duration = 3
            })
        end
...
Tabs.AD:AddButton({
    Name = Translator:traduzir("🔇 Parar Música"),
    Description = Translator:traduzir("Parar música atual"),
    Callback = function()
        ...
        redzlib:Notify({
            ...
            Description = "Música interrompida!",
            Type = "Warning",
            Duration = 3
        })
    end
...
Tabs.AD:AddToggle({
    Name = Translator:traduzir("🔄 Loop Música Especial"),
    ...
    Callback = function(value)
        ...
        if value then
            if selectedSpecialMusic then
                specialMusicThread = task.spawn(function()
                    while specialMusicLoop do
                        ...
                        -- Esperar antes de repetir (ajuste conforme necessário)
                        task.wait(15) -- 15 segundos entre repetições
                    end
                end)
                
                redzlib:Notify({
                    Title = "🔁 Loop Ativado",
                    Description = "Música em loop para o servidor!",
                    Type = "Success",
                    Duration = 3
                })
            else
            ...
            redzlib:Notify({
                ...
                Description = "Loop de música interrompido!",
                Type = "Warning",
                Duration = 3
            })
        end
...
-- Adicionar também para tocar no rádio do carro/casa
Tabs.AD:AddButton({
    Name = Translator:traduzir("🎶 Tocar no Rádio (GamePass)"),
    Description = Translator:traduzir("Tocar música no rádio do carro/casa"),
    Callback = function()
        if selectedSpecialMusic then
            local args = {
                [1] = "ToolMusicText",
            ...
            if remote then
                ...
                redzlib:Notify({
                    Title = "📻 Rádio",
                    Description = "Música tocando no rádio!",
                    Type = "Success",
                    Duration = 3
                })
            else
                redzlib:Notify({
                    Title = "❌ Erro Rádio",
                    Description = "RemoteEvent do rádio não encontrado!",
                    Type = "Error",
                    Duration = 3
                })
            end
